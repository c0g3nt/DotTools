...
{
    /// <summary>
    /// A SqlPackage.exe script action creates a Transact-SQL 
    /// incremental update script that updates the schema of a 
    /// target database to match the schema of a source database.
    /// </summary>
    [Description("A SqlPackage.exe script action creates a Transact-SQL " +
        "incremental update script that updates the schema of a target " +
    ...
    public class ScriptParameters : DeploymentParameters
    {
        ...
        public override DacActionValue Action { 
            get => DacActionValue.Script; 
            set => action = value; }

        /// <summary>
        /// Specifies an optional file path to output the 
        /// deployment script. For Azure deployments, if there are 
        /// TSQL commands to create or modify the master database, a script 
        /// will be written to the same path but with "Filename_Master.sql" 
        ...
        /// <summary>
        /// Specifies the file path where 
        /// the output files are generated.
        /// </summary>
        [Description("Specifies the file path where " +
            "the output files are generated.")]
        [DefaultValue(typeof(string), null)]
